...
def recent_prices(limit):
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute(
        "SELECT * FROM option_prices ORDER BY timestamp DESC LIMIT %s", (limit,)
    )
...
@app.route(
    "/api/latest/<string:expiry>/<string:strike>/<string:option_type>", methods=["GET"]
)
...
def get_latest_sentiment(ticker):
    ...
    buy_count = (
        latest_entry.get("buy", 0) + latest_entry.get("strongBuy", 0)
        if latest_entry
        else 0
    )
    hold_count = latest_entry.get("hold", 0) if latest_entry else 0
    sell_count = (
        latest_entry.get("sell", 0) + latest_entry.get("strongSell", 0)
        if latest_entry
        else 0
    )
